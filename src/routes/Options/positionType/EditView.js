  ...
  handleSubmit(e) {
    ...
    this.props.form.validateFieldsAndScroll((err, values) => {
      ...
      info.id = this.props.info.id;
        ...
        RecruitApi.updateOrAddPositionType(info, (resp) => {
          message.info(`${this.isAdd() ? '新增' : '编辑'}职业类型选项成功`);
          this.props.onDialogDismiss();
        }, (error) => {
          alert(JSON.stringify(error));
          message.info(`${this.isAdd() ? '新增' : '编辑'}职业类型选项失败: ${JSON.stringify(error)}`);
        });
