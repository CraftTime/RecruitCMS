...
import Style from '../../Options/style.less';
...
class JobStateListView extends Component {
  constructor(props) {
    super(props);
    this.state = {
      data: [],
    ...
    return (
      <div>
        ...
        <Modal
          ...
          destroyOnClose="true"
          title={isEmpty(info) ? '新增求职状态' : '编辑求职状态'}
  ...
  refreshList() {
    ...
    RecruitApi.listJobState(info, (resp) => {
      this.setState({
        data: resp.data,
      });
      ...
      message.error('获取求职状态失败: ' + JSON.stringify(error));
    });
  }

  onDelClick(id) {
    RecruitApi.deleteJobState(id, (resp) => {
      message.success('删除求职状态成功');
      this.refreshList();
    }, (error) => {
...
export default JobStateListView;
