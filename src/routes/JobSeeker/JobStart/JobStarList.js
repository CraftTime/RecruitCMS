...
class JobStarListView extends Component {
  constructor(props) {
    super(props);
    this.state = {
      data: [],
  ...
  refreshList() {
    ...
    RecruitApi.jobStarList(info, (resp)=> {
      for(let i = 0; i < resp.data.records.length; i++) {
        resp.data.records[i].starDate=this.dateFunction(resp.data.records[i].starDate);
      }
      this.setState({
        data: resp.data,
      });
...
export default JobStarListView;
