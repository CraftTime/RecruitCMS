...
import {updateCultureScoreOrder} from '../../../../services/AppApi';
...
class EditView extends Component {
			...
			>
				...
				>
					{getFieldDecorator('number', {
						initialValue: order.cultureCalculateCount,
						rules: [{
							required: true, message: '请输入测算次数',
						}],
					})(
						<text>{order.cultureCalculateCount}</text>
					)}
				</FormItem>
	...
	onChange(type, value) {
		...
		switch (type) {
			...
			case STATUS:
				...
				break;
			case NUMBER:
				newOrder.cultureCalculateCount = value;
				break;
		}
	...
	updatePriceInfo() {
		let info = {
			...
			cultureCalculateCount: this.state.order.cultureCalculateCount
		};

		let that = this;
		updateCultureScoreOrder(info).then(function (response) {
			if (isSuccess(response)) {
				message.success("文化分测算订单操作成功");
				that.props.onDialogDismiss();
			} else {
				message.error("文化分测算订单操作失败 error: " + JSON.stringify(response));
			}
		}).catch(function (error) {
			message.error("文化分测算订单操作失败 error: " + JSON.stringify(error));
		})
	}
}
